...
public class StateMachineNormalBolt extends BaseRichBolt {
	...
	public void execute(Tuple input) {
		...
		if (streamname == "item") {
			...
			_orders.get(warehouse_id).get(district_id)
					.put(order_id, order);
			++_numOrders;
			...
			for (int d_id = 1; d_id < BenchmarkConstant.DISTRICTS_PER_WAREHOUSE + 1; ++d_id) {
				...
				for (OrderLineState tmp : orderlineList) {
			...
			_districts.get(w_id).get(d_id)._next_o_id = d_next_o_id + 1;
			...
			Map<Integer, OrderState> tmpOrderList = _orders.get(w_id).get(
					d_id);
			...
			if (c_id != -1) {
				...
				// Get the midpoint customer's id
				List<CustomerState> customerList = new LinkedList<CustomerState>();
				for (CustomerState entry : _customers.get(c_w_id)
						.get(c_d_id).values()) {
			...
			WarehouseState tmpWarehouse = _warehouses.get(w_id);
			DistrictState tmpDistrict = _districts.get(w_id).get(d_id);
