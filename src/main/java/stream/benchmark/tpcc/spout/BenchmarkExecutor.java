...
public class BenchmarkExecutor {
	...
	public String generateDeliveryParams() {
		...
		int o_carrier_id = BenchmarkRandom.getNumber(
	...
	public String generateNewOrderParams() {
		...
		int ol_cnt = BenchmarkRandom.getNumber(BenchmarkConstant.MIN_OL_CNT,
		...
		for (int i = 0; i < ol_cnt; ++i) {
			...
			if (_scaleParams._warehouses > 1 && remote) {
				i_w_ids.add(BenchmarkRandom.getNumberExcluding(
						_scaleParams._starting_warehouse,
						_scaleParams._ending_warehouse, w_id));
			} else {
				i_w_ids.add(w_id);
			}
			i_qtys.add(BenchmarkRandom.getNumber(1,
	...
	public String generateOrderStatusParams() {
		...
		if (BenchmarkRandom.getNumber(1, 100) <= 60) {
			c_last = BenchmarkRandom
					.makeRandomLastName(_scaleParams._numCustomersPerDistrict);
		}
	...
	public String generatePaymentParams() {
		...
		int c_id = -1;
		String c_last = "";
		double h_amount = BenchmarkRandom.getFixedPoint(2,
				BenchmarkConstant.MIN_PAYMENT, BenchmarkConstant.MAX_PAYMENT);
		...
		// 15%: paying through another warehouse
			...
			c_w_id = BenchmarkRandom.getNumberExcluding(
					_scaleParams._starting_warehouse,
					_scaleParams._ending_warehouse, w_id);
	...
	public String generateStockLevelParams() {
		...
		int w_id = makeWarehouseId();
		int d_id = makeDistrictId();
		int threshold = BenchmarkRandom.getNumber(
				BenchmarkConstant.MIN_STOCK_LEVEL_THRESHOLD,
				BenchmarkConstant.MAX_STOCK_LEVEL_THRESHOLD);
		StringBuilder sb = new StringBuilder();
		sb.append(w_id);
		sb.append(",");
		sb.append(d_id);
	...
	private int makeWarehouseId() {
		return BenchmarkRandom.getNumber(_scaleParams._starting_warehouse,
				_scaleParams._ending_warehouse);
	}

	private int makeDistrictId() {
		return BenchmarkRandom.getNumber(1,
				_scaleParams._numDistrictsPerWarehouse);
	}

	private int makeCustomerId() {
		return BenchmarkRandom.NURand(1023, 1,
